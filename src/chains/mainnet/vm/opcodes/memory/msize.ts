...
export const msize: Omit<Opcode, 'examples'> = {
  ...
  notes: ['The memory is always fully accessible. What this instruction tracks is the highest offset that was accessed in the current execution. A first write or read to a bigger offset will trigger a memory expansion, which will cost gas. The size is always a multiple of a word (32 bytes).'],
  references: [
    {
      name: 'evm.codes',
    ...
    },
    {
      ...
      url: ethSpecsOpcodeSrc(MainnetHardforks.Shanghai, OpcodeGroups.Arithmetic, 120),
    },
  ],
