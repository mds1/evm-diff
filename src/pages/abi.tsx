  ...
  return (
    <>
      ...
      <Tab.Group selectedIndex={selectedTab} onChange={onTabChange}>
        <Tab.List className='space-x-4 overflow-x-auto whitespace-nowrap border-b border-zinc-200 dark:border-zinc-700 md:flex md:justify-center md:space-x-8'>
          {tabs.map((tab) => {
            return (
                ...
                {({ selected }) => (
                  <div
                    className={classNames(
                      ...
                      selected
                        ? 'text-accent border-b-2 border-b-green-800 outline-none dark:border-b-green-300'
        ...
        <Tab.Panels className='text-center' style={{ opacity: isLoading ? 0 : 1 }}>
          {tabs.map((tab) => {
            return (
              ...
              >
                <button
                  className='absolute right-3 top-4 z-10 mr-10 rounded-md border border-zinc-500 p-1 hover:border-black focus:outline-0 dark:border-zinc-400 hover:dark:border-zinc-200'
                  style={{
                ...
                <button
                  className='absolute right-3 top-4 z-10 rounded-md border border-zinc-500 p-1 hover:border-black focus:outline-0 dark:border-zinc-400 hover:dark:border-zinc-200'
                  style={{
                    // Blur the background behind the copy button.
                    background: 'rgba(0, 0, 0, 0.0)',
                    backdropFilter: 'blur(4px)',
